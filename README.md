...
## 其他常用命令
- gnb终端处:ng(查看基站是否连接核心网)、t（打印速率等信息）、cell(查看基站下属小区)
- mme终端处：ue(查看接入核心网的终端，含imsi、ip等信息)、gnb(查看接入核心网的基站)
- 灌包：```iperf -u -c 192.168.2.3 -i 1 -b 1G -t 100```  （UDP包，192.168.2.3为灌包目的IP、-i为打印时间间隔(s)、-b为发包速率、-t为灌包时长（s））
